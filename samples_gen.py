...
def mesh_2D(Nx, Nt, range_x, range_t):
    ...
    collocation_points = torch.cartesian_prod(x, t).float()

    interior_point = torch.cartesian_prod(x[1: -1], t[1: -1]).float()
    u_initial = torch.hstack((x.view(-1, 1), torch.zeros((Nx, 1)))).float()
    'initial condition u(x,0)'

    u_boundary_left = torch.hstack((torch.full([Nt, 1], range_x[0]), t.view(-1, 1))).float()
    'left boundary condition u(0,t)'

    u_boundary_right = torch.hstack((torch.full([Nt, 1], range_x[1]), t.view(-1, 1))).float()
